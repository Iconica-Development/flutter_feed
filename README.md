...
Flutter Feed is a package which shows a list posts by a user. This package also has additional features like liking a post and leaving comments. Default this package adds support for a Firebase back-end.

![Flutter Feed GIF](example.gif)

## Setup
To use this package, add flutter_feed as a dependency in your pubspec.yaml file:

```
  flutter_feed:
    git: 
      url: https://github.com/Iconica-Development/flutter_feed.git
      path: packages/flutter_feed
```

If you are going to use Firebase as the back-end of the Feed, you should also add the following package as a dependency to your pubspec.yaml file:

```
  flutter_feed_firebase:
    git: 
      url: https://github.com/Iconica-Development/flutter_feed.git
      path: packages/flutter_feedfirebase
```

In Firebase add Firestore and storage to your project.
In Firestore add a collection named `feed` and a collection named `users`.
In the `feed` collection all posts will be stored. In the `users` collection all users will be stored.
...
````
feedNavigatorUserStory(FeedUserStoryConfiguration, context),
````


Or create your own routing using the Screens:
To add the `FeedScreen` add the following code:
...
`FeedScreen` is supplied with a standard `FeedPostScreen` which opens the detail page of the selected post. Needed parameter like `FeedService` and `FeedOptions` will be the same as the ones supplied to the `FeedScreen`.

The standard `FeedPostScreen` can be overridden by defining `onPostTap` as shown below.
...
A standard post creation is provided named: `FeedPostCreationScreen`. Routing to this has to be done manually.

```
FeedPostCreationScreen(
...
The `FeedOptions` has its own parameters, as specified below:
...
| feedPostHeight | Sets the height for each post widget in the list of post. If null, the size depends on the size of the image. |
...
| enablePostOverviewScreen | Boolean to enable feed post overview screen before submitting. |
...
Please file any issues, bugs or feature request as an issue on our [GitHub](https://github.com/Iconica-Development/flutter_feed/pulls) page. Commercial support is available if you need help with integration with your app or services. You can contact us at [support@iconica.nl](mailto:support@iconica.nl).
...
This `flutter_feed` for Flutter is developed by [Iconica](https://iconica.nl). You can contact us at <support@iconica.nl>