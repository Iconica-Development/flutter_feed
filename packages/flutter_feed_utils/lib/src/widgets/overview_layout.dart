...
class OverviewLayout extends StatelessWidget {
  const OverviewLayout._({
    required _OverviewLayoutOption activeLayout,
    required this.items,
    this.columnCount = 1,
  ...
  factory OverviewLayout.list({required List<Widget> children}) =>
      OverviewLayout._(
        activeLayout: _OverviewLayoutOption.list,
        items: children,
      );
  ...
  factory OverviewLayout.grid({
    ...
    required List<Widget> children,
      ...
      OverviewLayout._(
        activeLayout: _OverviewLayoutOption.grid,
        columnCount: columnCount,
        items: children,
      );

  final List<Widget> items;
  final _OverviewLayoutOption _activeLayout;
  final int columnCount;

  @override
  Widget build(BuildContext context) => switch (_activeLayout) {
        _OverviewLayoutOption.list => SingleChildScrollView(
            child: Padding(
        ...
        _OverviewLayoutOption.grid => SingleChildScrollView(
            child: Padding(
              padding: const EdgeInsets.symmetric(horizontal: 12.0),
              child: Column(
                children: [
